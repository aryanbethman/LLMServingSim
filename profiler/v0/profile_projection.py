...
ROOT = REPO / "profiler" / "v0" / "perf_models"
REQUIRED_LAYERS = {
...
def validate_profile(profile: Path, target_tp: int) -> dict[str, Any]:
    ...
    return {
        "profile": str(profile.relative_to(REPO) if profile.is_relative_to(REPO) else profile),
        "target_tp": target_tp,
...
def load_model(model: str) -> dict[str, Any]:
    return json.loads((REPO / "configs" / "model" / f"{model}.json").read_text())

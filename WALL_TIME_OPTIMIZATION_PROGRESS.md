...
## Final comparison (2026-09-19/20)

Every run below: ShareGPT-750, one run at a time on an otherwise idle host, both modes from the same frozen snapshot of the current code (LLMServingSim `07dd7b1`, astra-sim `b87c9e2`, chakra `96bdc9c`; verified identical to the working tree). Sequential from 21:19 to 00:31 UTC, no overlap. Results in `~/hipc-results/final-*-20260920/`.

| NPUs | Mode | Wall | Python CPU | ASTRA-Sim CPU | Tree RSS | FS output | Open FDs (steady) | CSV / clock |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| 16 (4×TP4) | legacy | 4:04.96 | 124 s | 118 s | 213 MiB | 2.88 GB | 38 | — |
| 16 (4×TP4) | shared | **3:27.03** (−15.5%) | 80 s | 126 s | 205 MiB | 2.4 MB | 21 | identical |
| 72 (9×TP8) | legacy | 29:50.27 | 704 s | 1,072 s | 366 MiB | 16.9 GB | 94 | — |
| 72 (9×TP8) | shared | **23:43.05** (−20.5%) | 166 s | 1,251 s | 350 MiB | 3.0 MB | 21 | identical |
| 405B (1×TP8×PP2) | legacy | 3:42.98 | 79 s | 145 s | 217 MiB | 2.60 GB | 38 | — |
| 405B (1×TP8×PP2) | shared | **3:42.01** (−0.4%) | 85 s | 138 s | 213 MiB | 2.5 MB | 21 | identical |
| 256 (64×TP4) | legacy | 1:20:14 | 1,833 s | 3,019 s | 642 MiB | 39.8 GB | 277 | — |
| 256 (64×TP4) | shared | **42:21.97** (−47.2%) | 196 s | 2,349 s | 560 MiB | 4.8 MB | 21 | identical |

Every pair produced a byte-identical `requests.csv` and simulated clock, and every run exited 0. The 16-NPU clock is the 89,678,987,869 ns baseline; 256 NPUs is a new configuration (`baseline_scale_256.json`, 64 TP4 replicas, generated by `analysis/generate_scale_configs.py`).

Shared-template transport per run:

| NPUs | Bundles | Templates sent | Wire | Bindings cache hit/miss | Relocated hits | Cache size | ASTRA template-cache high water |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 16 | 31,872 | 5,416 | 359 MB | 8,347 / 7,323 | 5,611 | 8.2 MB | 132 |
| 72 | 93,768 | 15,336 | 1,085 MB | 39,158 / 6,753 | 35,101 | 14.2 MB | 136 |
| 405B | 8,800 | 13,712 | 717 MB | 1,749 / 2,614 | 0 | 16.3 MB | 144 |
| 256 | 399,734 | 3,896 | 531 MB | 195,713 / 3,597 | 193,484 | 4.0 MB | 132 |

### What the numbers say

- **The advantage grows with instance count**: parity on the single-instance 405B case, −15.5% at 16 NPUs (4 instances), −20.5% at 72 (9), −47.2% at 256 (64). The bindings cache relocates a batch's bindings across identical instances, so its hit rate rises with replica count: 52% at 16 NPUs, 85% at 72, 98% at 256. On 405B there is one instance and its PP overlays are not relocatable, so it hits 40%.
- **Python is what scales**: at 256 NPUs legacy spends 1,833 s against 196 s, a 9.4x difference, and it is the whole wall-clock gap there (ASTRA-Sim: 3,019 s vs 2,349 s).
- **Filesystem**: legacy wrote 39.8 GB at 256 NPUs, leaving 18.2 GiB free on this shared host, and holds one open ET per NPU for the whole run (277 FDs). Shared wrote 4.8 MB and holds 21 FDs; its 213-FD peak is a startup transient while the 64 instance configs are written (21 for 243 of 250 samples).
- **Memory** is slightly lower in shared mode at every scale (560 vs 642 MiB at 256 NPUs), with the ASTRA-Sim template cache bounded at 128 entries (high water 132-144).
- 256-NPU legacy finished here, but it came within 6 GiB of the disk floor. Nothing guarantees that on a fuller disk; the run needs ~40 GB of scratch, and the next size up would not fit.
